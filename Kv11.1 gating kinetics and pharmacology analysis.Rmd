...
#Select only sweep at P1 40mV
library(dplyr)
library(tidyr)

Fractionalblock_flecainide <- Pharmacology_Equine_Human %>%
  filter(Compound %in% c("Flecainide Acetate", "Vehicle","None")) %>%
  filter(Membrane_voltage == 40) %>%
  filter(Compound_conc_µM %in% c(0, 1.5)) %>%
  group_by(Cell) %>%
    filter(n_distinct(Compound_conc_µM) == 2) %>%
    ungroup() %>%  #only use cells that both have a compound and vehicle control
  select(Cell, Species, Compound_conc_µM, Peak_tail_current) %>%
  pivot_wider(
    names_from = Compound_conc_µM,
    values_from = Peak_tail_current,
    names_prefix = "tailcurrent_"
  ) %>%
  mutate(
    fraction = tailcurrent_1.5 / tailcurrent_0,
...
ggplot(Fractionalblock_flecainide, aes(x = Species, y = percent_block, color = Species, fill=Species)) +
  geom_boxplot(coef = 1.5, width = 0.6,lwd = 0.7,staplewidth = 0.1) +
  stat_summary(fun=mean, geom="point",shape=8, size=4,col="black")+
  labs(
    title = "Fractional Block at 1.5 µM Flecainide",
    y = "Percent block (%)",
    x = "Species") +
  theme_classic() +
...
wilcox.test(percent_block ~ Species,
            data = Fractionalblock_flecainide,
            alternative = "greater")
```
