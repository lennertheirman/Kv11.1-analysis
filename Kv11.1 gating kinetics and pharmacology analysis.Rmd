...
library(readxl)
Tailcurrent_Equine_Human <- read_excel("Kv11.1_HEK293t_Equine_Human_Peak tail current_long format - Copy.xlsx")
...
library(dplyr)
df_norm <- Tailcurrent_Equine_Human %>%
  group_by(Recording) %>%   # normalize per recording
  mutate(
    max_tailcurrent = max(`Baseline_substracted_peak_tail_current`, na.rm = TRUE),
    norm_tailcurrent = `Baseline_substracted_peak_tail_current` / max_tailcurrent
  ) %>%
  ungroup()
View(df_norm)
...
V05_guess <- summary_df$Membrane_voltage[ which.min(abs(summary_df$mean_Inorm - 0.5)) ]
k_guess   <- (max(summary_df$Membrane_voltage) - min(summary_df$Membrane_voltage)) / 8


Boltzmann_equine <- nls(
  mean_Inorm ~ Boltzmann(Membrane_voltage, V05, k),
  data  = filter(summary_df, Species == "Equine"),
  start = list(V05 = V05_guess, k = k_guess))
...
ggsave("IV_curve_Kv11.1_Equine_Human.png", width = 10, height = 3, dpi = 600)
